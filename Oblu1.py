...
device_ip = '172.31.5.211'
#device_ip2 = '172.31.5.211'
...
SYS_OFF_CMD = "220022"
PROC_OFF_CMD = "320032"
LOG_FILE = "stepsL.txt" #check in powershell, > Get-Content -Path "C:/xampp/htdocs/iot_single/steps" -Wait
TIME_OUT = 10
NUM_RETRY = -1
g_isRunning = True
queue = Queue.Queue()
...
class DeviceClient(threading.Thread):
	...
	cov6=sensor_data[11]
        ...
        return dx, dy, dz, dp, disp, cov1, cov2, cov3, cov4, cov5, cov6
    ...
    def write_file_hdr(self):
        ...
        if self.outfile :
            #str = "%10s\t%6s\t%6s\t%6s\t%6s\n" % ("PKT No.", "X", "Y", "Z", "Angle(Degree), cov1, cov2, cov3, cov4, cov5, cov6")
	    str=""
            self.outfile.write(str)
            self.outfile.flush()

    def write_file(self, x, y, z,phi, cov1, cov2, cov3, cov4, cov5, cov6):
        dx = self.prev_x - x
        dy = self.prev_y - y
        distance = math.sqrt(dx * dx + dy * dy)
        if (distance > 0.05):
            if self.outfile :
                str = "%d, %0.2f, %0.2f, %0.2f, %0.2f, %0.2f, %0.20f, %0.20f, %0.20f, %0.20f, %0.20f\n" % (self.pkt_counter, -x, y, -z, phi, cov1, cov2, cov3, cov4, cov5, cov6 )
                self.outfile.write(str)
                self.outfile.flush()
                queue.put(str)
                self.prev_x = x
                ...
                while g_isRunning:
                    try :
                        ...
                        if curr_pkt != prev_pkt:
                            ...
                            dx, dy, dz, dp, disp, cov1, cov2, cov3, cov4, cov5, cov6 = self.calc_disp(payload, phi)
                            xpos += dx
                            ypos += dy
                            zpos += dz
                            phi += dp
                            ...
                            # print count
                            prev_pkt = curr_pkt
                            self.write_file(xpos, ypos, zpos, phi, cov1, cov2, cov3, cov4, cov5, cov6)
